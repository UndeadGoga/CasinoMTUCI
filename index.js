...
const WEB_URL = PORT === 1921 ? "http://localhost:1921/" : "https://blackjack/";
...
server.listen(PORT, () => {
    console.log(`Listening on ${process.env.PORT} or 1921`)
...
app.post('/api', async (req, res) => {
    await pool.query("CREATE TABLE IF NOT EXISTS cas_users(email text, password text, nickname text, balance int)"); // it's a migration
    const { nickname, email, password, action } = req.body;

    try {
        if (action === 'signup') {
            // Регистрация
            const newUser = await pool.query(
                "INSERT INTO cas_users (nickname, email, password, balance) VALUES ($1, $2, $3, $4) RETURNING *",
                [nickname, email, password, 3000]
            );
            ...
            res.redirect("http://localhost:1921/index")

        } else if (action === 'login') {
            // Вход
            const user = await pool.query("SELECT * FROM cas_users WHERE email = $1", [email]);
            current_user = user.rows[0]
            console.log(current_user)
            ...
            res.status(400).json("Недопустимое действие");
        }
    } catch (err) {
...
wss.on("connection", (ws) => {
    ...
    ws.on("message", (message) => {
        ...
        if (result.method === "join") {
            const nickname = current_user["nickname"];
        ...
        if (result.method === "hasLeft") {
            ...
            players.forEach((_player) => {
                if (_player["nickname"] == current_user["nickname"]) {
                    cur = _player

                    pool.query("UPDATE cas_users set balance = $1", [_player["balance"]])
                }
            })


            pool.query("UPDATE cas_users set balance = $1", [cur["balance"]])
        ...
        if (result.method === "finalCompare") {
            ...
            // pool.query("UPDATE cas_users set balance = $1", [])
        ...
        if (result.method === "getRoute") {
            ...
            for (let i = 3; i < app._router.stack.length; i++) {
                if (app._router.stack[i].route === "/" + getRouteId) {
                    isRouteDefined = true;
                } else {
...
app.get("*", async function (req, res) {
    ...
    console.log(cookies)
    let email = cookies["password"]
    const user_ = await pool.query("SELECT * FROM cas_users WHERE password = $1", [email])
    current_user = user_.rows[0]
    console.log(current_user)
    res.redirect("/");
});
